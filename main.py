                ...
                if self.on_quota_exhausted_callback:
                    try:
                        self.on_quota_exhausted_callback(
                            ...
                            "بات همچنان عادی کار می‌کنه و سیگنال می‌ده، ولی تا برگشتن سهمیه:\n"
                            "• لایه‌ی قضاوت هوشمند معامله غیرفعاله (تایید فقط بر پایه‌ی امتیاز کمی)\n"
                            "• تنظیم پارامتر دوره‌ای متوقفه (آخرین پارامترهای تنظیم‌شده توسط AI همچنان استفاده می‌شن)"
                        )
                    except Exception:
                        pass
            logger.warning(f"{symbol}: سهمیه‌ی رایگان Groq تموم شده - سیگنال فقط بر پایه امتیاز کمی تایید می‌شه (بدون قضاوت AI).")
            return {"approve": True, "confidence": self.config.MIN_JUDGE_CONFIDENCE,
                    "reason": "سهمیه‌ی رایگان Groq تموم شده - تایید صرفاً بر پایه امتیاز کمی"}
